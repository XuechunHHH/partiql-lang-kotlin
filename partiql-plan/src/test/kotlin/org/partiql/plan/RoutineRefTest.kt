...
class RoutineRefTest {
    ...
    private val routineRef = RoutineRef(
        RoutineId("com.example.provider.mixedCaseRoutine"),
        "Catalog",
        Name.of("Namespace", "Routine"),
    )
    ...
    fun isAnExactValue() {
        ...
        assertEquals(Name.of("Namespace", "Routine"), routineRef.name)
    }
    ...
    fun identityAwareFactoriesAttachIdentity() {
        ...
        assertSame(routineRef, Operators.STANDARD.call(function, listOf(literal), routineRef).routineRef)
        assertSame(
            routineRef,
            Operators.STANDARD.dispatch("routine", listOf(overload), listOf(literal), routineRef).routineRef,
        )
    }
    ...
    @Test
    fun measureCopyPreservesIdentity() {
