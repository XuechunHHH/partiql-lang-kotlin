...
internal class TestRoutineCatalog private constructor(
    ...
    private val routines: RoutineProvider,
    ...
    ) = routines.getFunctions(identifier).also {
    ...
    override fun resolveAggregations(
        session: Session,
        identifier: Identifier,
    ) = routines.getAggregations(identifier).also {
    ...
    internal class Builder(private val name: String) {
        ...
        private var routines: RoutineProvider? = null
        ...
        internal fun routines(routines: RoutineProvider): Builder = apply {
            this.routines = routines
        }

        internal fun build(): TestRoutineCatalog {
            val configured = routines
            require(configured == null || (functions.isEmpty() && aggregations.isEmpty())) {
                "A test catalog cannot combine a supplied provider with direct test bindings"
            }
            val provider = configured ?: object : RoutineProvider {
                override fun getFunctions(identifier: Identifier): Collection<RoutineBinding<FnOverload>> =
                    functions.filter { it.canonicalName.matches(identifier) }

                override fun getAggregations(identifier: Identifier): Collection<RoutineBinding<AggOverload>> =
                    aggregations.filter { it.canonicalName.matches(identifier) }
            }
            return TestRoutineCatalog(name, provider, tables)
        }
    }
}
