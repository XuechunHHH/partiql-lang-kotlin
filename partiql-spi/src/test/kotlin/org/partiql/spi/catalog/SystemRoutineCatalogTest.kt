...
class SystemRoutineCatalogTest {
    ...
    fun systemCatalogImplementsExactRoutineContract() {
        ...
        assertFalse(catalog.getRoutineInventory().functions.isEmpty())
        assertFalse(catalog.getRoutineInventory().aggregations.isEmpty())
    }
    ...
    @Test
    fun inventoryIsCompleteRootOnlyAndDeterministicallyOrdered() {
        val inventory = catalog.getRoutineInventory()

        assertTrue(inventory.functions.all { !it.canonicalName.hasNamespace() })
        assertTrue(inventory.aggregations.all { !it.canonicalName.hasNamespace() })
        assertEquals(
            inventory.functions.map { it.canonicalName }.sortedWith(NAME_COMPARATOR),
            inventory.functions.map { it.canonicalName },
        )
        assertEquals(
            inventory.aggregations.map { it.canonicalName }.sortedWith(NAME_COMPARATOR),
            inventory.aggregations.map { it.canonicalName },
        )
        inventory.functions.forEach { binding ->
            assertEquals(
                listOf(binding.canonicalName),
                catalog.resolveFunctions(
                    session,
                    Identifier.delimited(binding.canonicalName.getName()),
                ).map { it.canonicalName },
            )
            assertEquals(
                catalog.getFunctions(session, binding.canonicalName.getName()),
                binding.overloads,
            )
        }
        inventory.aggregations.forEach { binding ->
            assertEquals(
                listOf(binding.canonicalName),
                catalog.resolveAggregations(
                    session,
                    Identifier.delimited(binding.canonicalName.getName()),
                ).map { it.canonicalName },
            )
            assertEquals(
                catalog.getAggregations(session, binding.canonicalName.getName()),
                binding.overloads,
            )
        }
    }
    ...
    private companion object {
        private val NAME_COMPARATOR: Comparator<Name> = Comparator { first, second ->
            ...
            firstParts.size.compareTo(secondParts.size)
        }
    }
}
