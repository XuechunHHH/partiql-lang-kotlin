 ...
 * parameter signatures compose under one exact name and routine kind.
 */
public class MemRoutineProvider private constructor(
    ...
    public class Builder internal constructor() {
        ...
        /**
         * Registers [fn] under the complete catalog-local [name].
         */
        public fun register(
            fn: FnOverload,
            name: Name,
        ...
        /**
         * Registers [agg] under the complete catalog-local [name].
         */
        public fun register(
            agg: AggOverload,
            name: Name,
        ...
        public fun build(): MemRoutineProvider {
            validateCanonicalNames((functions.map { it.name } + aggregations.map { it.name }).distinct())
        ...
        private fun validateCanonicalNames(names: List<Name>) {
            ...
            names.forEachIndexed { index, name ->
                val conflicting = names.take(index).firstOrNull {
                    it != name && regularEquivalent(it, name)
                }
        ...
        private fun <T> bindings(
            ...
            return Collections.unmodifiableList(
                byName.map { (name, overloads) -> RoutineBinding(name, overloads) },
            )
        }
