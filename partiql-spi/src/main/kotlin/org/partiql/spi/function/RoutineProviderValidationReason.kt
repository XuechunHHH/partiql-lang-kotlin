...
public enum class RoutineProviderValidationReason {
    /** A provider inventory callback or its returned declaration metadata could not be read. */
    PROVIDER_ACCESS_FAILED,

    /** A provider source name contains an empty segment. */
    EMPTY_SOURCE_SEGMENT,

    /** One provider declares the same opaque routine ID more than once. */
    DUPLICATE_ROUTINE_ID,

    /** One provider declares the same exact source name more than once. */
    DUPLICATE_SOURCE_NAME,

    /** A provided routine has no overloads. */
    EMPTY_OVERLOADS,

    /** An overload signature name does not exactly equal its provider source-name leaf. */
    SIGNATURE_NAME_MISMATCH,

    /** One routine has multiple overloads with the same ordered parameter types. */
